                ...
                if (CheckNetworkAvailibility.isConnectionAvailable(getApplicationContext())) {
                    if (validate()) {
                        if (user.equals("student")) {
                            DatabaseReference LOGINRECORD = FirebaseDatabase.getInstance().getReference().child("School").child("LoginRecord");

                            LOGINRECORD.child(registrationNumber.getText().toString()).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (snapshot.exists())
                                    {
                                            LOGINRECORD.child(registrationNumber.getText().toString()).addChildEventListener(new ChildEventListener() {
                                                @Override
                                ...
                                public void onCancelled(@NonNull DatabaseError error) {
                                    progressBar.setVisibility(View.GONE);
                                    Toast.makeText(MainActivity.this, "Unable to login : "+error.getMessage(), Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
